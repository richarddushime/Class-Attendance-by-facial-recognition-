...
#used	
def this_week_stud_count_vs_date():
	today=datetime.date.today()
	...
	while(cnt<5):
		...
		if(str_dates.count(date))>0:
			...
			stud_cnt_all.append(stud_count[idx])
		else:
			stud_cnt_all.append(0)
	...
	df["Number of students"]=stud_cnt_all
...
#used
def last_week_stud_count_vs_date():
	today=datetime.date.today()
	some_day_last_week=today-datetime.timedelta(days=7)
	monday_of_last_week=some_day_last_week-  datetime.timedelta(days=(some_day_last_week.isocalendar()[2] - 1))
	monday_of_this_week = monday_of_last_week + datetime.timedelta(days=7)
	...
	df=pd.DataFrame()
	df["date"]=str_dates_all
	df["stud_count"]=stud_cnt_all
	

	
	
	sns.lineplot(data=df,x='date',y='stud_count')
...
def view_attendance_home(request):
	total_num_of_stud=total_number_students()
	stud_present_today=students_present_today()
	this_week_stud_count_vs_date()
	last_week_stud_count_vs_date()
	return render(request,"recognition/view_attendance_home.html", {'total_num_of_stud' : total_num_of_stud, 'stud_present_today': stud_present_today})
